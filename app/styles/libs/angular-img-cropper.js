            ...
            link: function (scope, element) {
                ...
                var DragMarker = (function (_super) {
                    ...
                    DragMarker.prototype.getDragIconPoints = function (arr, scale) {
                        var maxLength = 34 * scale;
                        var arrowWidth = 28 * scale;
                        var arrowLength = 16 * scale;
                        var connectorThroat = 8 * scale;
...
//                        if ($rootScope.canvasCurrent === 1) {
//                            $rootScope.posting_img1 = croppedImg.src;
//                        } else if ($rootScope.canvasCurrent === 2) {
//                            $rootScope.posting_img2 = croppedImg.src;
//                        } else if ($rootScope.canvasCurrent === 3) {
//                            $rootScope.posting_img3 = croppedImg.src;
//                        } else if ($rootScope.canvasCurrent === 4) {
//                            $rootScope.posting_img4 = croppedImg.src;
//                        }
                    ...
                    ImageCropper.prototype.onTouchEnd = function (e) {
                        if (crop.isImageSet()) {
                            ...
                            if (crop.isImageSet() && this.currentlyInteracting) {
                                ...
                                if ($rootScope.canvasCurrent == 1) {
                                    $rootScope.posting_img1 = img.src;
                                } else if ($rootScope.canvasCurrent == 2) {
                                    $rootScope.posting_img2 = img.src;
                                } else if ($rootScope.canvasCurrent == 3) {
                                    $rootScope.posting_img3 = img.src;
                                } else if ($rootScope.canvasCurrent == 4) {
                                    $rootScope.posting_img4 = img.src;
                                }
                                scope.$apply();
                            }
                            ...
                            if (newValue != null) {
                                ...
                                imageObj.addEventListener("load", function () {
                                    ...
                                    if ($rootScope.isEditing != '') {
                                        scope.croppedImage = img.src;
                                    } else if ($rootScope.posting_img4 == '') {
                                        $rootScope.act_img4 = newValue;
                                        $rootScope.posting_img4 = img.src;
                                        $rootScope.canvasCurrent = 4;
                                        $rootScope.totalFilesAdded = $rootScope.totalFilesAdded + 1;
                                        scope.croppedImage = img.src;
                                    } else if ($rootScope.posting_img3 == '') {
                                        $rootScope.act_img3 = newValue;
                                        $rootScope.posting_img3 = img.src;
                                        $rootScope.canvasCurrent = 3;
                                        $rootScope.totalFilesAdded = $rootScope.totalFilesAdded + 1;
                                        scope.croppedImage = img.src;
                                    } else if ($rootScope.posting_img2 == '') {
                                        $rootScope.act_img2 = newValue;
                                        $rootScope.posting_img2 = img.src;
                                        $rootScope.canvasCurrent = 2;
                                        $rootScope.totalFilesAdded = $rootScope.totalFilesAdded + 1;
                                        scope.croppedImage = img.src;
                                    } else if ($rootScope.posting_img1 == '') {
                                        $rootScope.act_img1 = newValue;
                                        $rootScope.posting_img1 = img.src;
                ...
                $rootScope.doWatch = function (newValue) {
                    if (newValue != null) {
                        var imageObj = new Image();
                        imageObj.addEventListener("load", function () {
                            crop.setImage(imageObj);
...
angular.module('angular-img-cropper').directive("button2", ['$rootScope', function ($rootScope) {
        return{
            restrict: 'A',
            scope: {
                image: "="
            ...
            link: function (scope, element) {
                element.bind("click", function (e) {
                    $rootScope.isEditing = e.currentTarget.attributes.itemnum.value.trim();
                    $rootScope.canvasCurrent = $rootScope.isEditing;
                    $rootScope.doWatch(e.currentTarget.attributes.actimg.value);
                });
            }
        }
    }]);
angular.module('angular-img-cropper').directive("imgCropperFileread", ['$timeout', '$rootScope', function ($timeout, $rootScope) {
        return {
            scope: {
                image: "="
            },
            link: function (scope, element) {
                ...
                element.bind("change", function (changeEvent) {
                    jQuery('.canvas_loadings').show();
                    jQuery('.crop_continue').show();
                    var multipleUpTotal = changeEvent.target.files.length;
                    var reader1 = new FileReader();
                    reader1.onload = function (loadEvent) {
                        $timeout(function () {
                            $rootScope.doWatch(loadEvent.target.result);
                            //scope.image = loadEvent.target.result;
                        }, 0);
                    };
                    var reader2 = new FileReader();
                    reader2.onload = function (loadEvent) {
                        $timeout(function () {
                            $rootScope.doWatch(loadEvent.target.result);
                            //scope.image = loadEvent.target.result;
                        }, 500);
                    ...
                    reader3.onload = function (loadEvent) {
                        $timeout(function () {
                            $rootScope.doWatch(loadEvent.target.result);
                            //scope.image = loadEvent.target.result;
                        }, 1000);
                    ...
                    reader4.onload = function (loadEvent) {
                    ...
                    if (multipleUpTotal > 4 || $rootScope.totalFilesAdded == 4 || (multipleUpTotal + $rootScope.totalFilesAdded) > 4) {
                        alert("You can select maximum of only 4 images!!");
                        jQuery('.canvas_loadings').hide();
                        return false;
                    } else {

                        angular.forEach(changeEvent.target.files, function (item, i) {
                            var myfile = changeEvent.target.files[i];
                            if (myfile.type == 'image/jpeg' || myfile.type == 'image/png') {
                                if (myfile) {
                                    if (i == 0) {
                                        reader1.readAsDataURL(myfile);
                                    }
                                    if (i == 1) {
                                        reader2.readAsDataURL(myfile);
                                    }
                                    if (i == 2) {
                                        reader3.readAsDataURL(myfile);
                                    }
                                    if (i == 3) {
                                        reader4.readAsDataURL(myfile);
                                    }
